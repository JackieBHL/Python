...
class TN():
   ...
   # True if there is a full row of symbol 'symb'    
    ...
    def maximin_O(self):
        ...
        hypo_Oboards=[]
        for markO in places_for_O:
            hypo_ttt = copy.deepcopy(self.ttt)
            hypo_ttt[markO] = '@' # use@ for O
            hypo_Oboards.append([markO, hypo_ttt]) # hypo_ttt resulted from placing O at markO
        evaled_Oboards = []
        for (markO,oboard) in hypo_Oboards:
            maxeval = self.get_maxeval_among_Xboards(oboard)
            evaled_Oboards.append([maxeval,markO])

        best_place_for_O = min(evaled_Oboards)[1] 
        # finish with:
